<script lang="ts">
  ...
  const searchQuery = $derived(normalize($themeSearchInput));
  const filteredThemes = $derived(
    data.themes.filter((theme) => normalize(theme.name).includes(searchQuery))
  );
...
<div class="flex flex-wrap justify-center gap-0 p-2 pt-0 sm:gap-8 sm:pt-8">
  {#if filteredThemes.length === 0}
    <p class="px-4 py-10 text-center text-sm text-gray-300">No themes found.</p>
  {:else}
